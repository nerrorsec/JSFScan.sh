...
subjs -i "$target" | anew -q jsfile_links.txt
...
interlace -tL live_jsfile_links.txt -threads 50 -c "echo 'Scanning _target_ Now' ; python3 ./tools/LinkFinder/linkfinder.py -d -i _target_ -o cli >> endpoints.txt" -v
}

#Gather Secrets From Js Files
secret_js(){
echo -e "\n\e[36m[\e[32m+\e[36m]\e[92m Started Finding Secrets in JSFiles\e[0m\n";
interlace -tL live_jsfile_links.txt -threads 50 -c "python3 ./tools/SecretFinder/SecretFinder.py -i _target_ -o cli >> jslinksecret.txt" -v
}
...
interlace -tL live_jsfile_links.txt -threads 50 -c "bash ./tools/findomxss.sh _target_" -v
}
